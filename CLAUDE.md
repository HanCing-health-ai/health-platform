...
## 十五、Antigravity 會話開收工 SOP 與省 Token 指南

為最大化效能並節省 Token，請遵守以下流程：
1. **開工準備 (Session Start)**：
   - 確認目標明確。帶著明確的 Wave/任務單進入對話。
   - 剛進入對話時，Antigravity 會自動載入 `CLAUDE.md` 作為全域規則。
   - 如果遇到複雜架構變更，可先要求「切換至 Plan Mode」，讓我先草擬架構與步驟，確認無誤後再開始撰寫程式碼。
2. **開發進行中 (In Progress)**：
   - **Bug 修復停損點**：如果同一個 Bug 被我（Antigravity）連續修復 3 次仍未解決，請果斷中斷對話，將錯誤訊息帶回給「Claude 策略端」或「Claude Code 測試端」進行交叉除錯，避免在同一個迴圈浪費 Token。
   - **指令時機**：需要清理上下文時，優先使用 `/compact`（壓縮記憶）而非 `/clear`（完全失憶），以保留專案脈絡。
3. **收工檢核 (Session End)**：
   - 每次下線或 Wave 結束前，由我（Antigravity）主動更新 `CLAUDE.md` 的版本進度區塊。
   - **Token 榨乾策略**：如果該次會話的上下文額度還有剩餘，可指示我進行一次性的 `/arch`（架構健診）、`/cso`（資安掃描）或 E2E 測試，將多餘的 Token 轉換為專案防護力。

---

## 十六、V2/V3 短中長期評估備忘錄 (從 Claude 策略端提煉)

本區塊記錄在開發過程中萃取出的商業與架構價值，供未來 Wave 參考：
- **護城河敘事優化（企劃書 V5.0）**：參考 Grab 的雙引擎模型，將系統描述改為「知識飛輪 + 資料飛輪」的動態互強化敘事（越多師傅使用 → 知識庫越豐富 → AI 輸出越精準 → 師傅越依賴）。
- **資料資產商業化 (RMN 概念)**：累積匿名化行為模式資料後，系統本身即為「調理產業洞察平台」，可發展類似 Retail Media Network 的高價值精準行銷模型（V3 ESG 模組）。
- **工作流優化 (n8n Subagents)**：評估在 n8n 中使用 Subagents 進行知識庫**並行大量入庫**，取代現有的線性流水線，以加速處理效率。
- **持久化 Agent 升級**：評估將目前 `.claude/commands` 下的角色指令（/dev, /km, /arch, /design）升級為具有獨立記憶、可指派顏色的持久化 Agents。
- **Obsidian 決策日誌架構**：未來 V2 考慮導入 `projects / inbox / decisions` 的 Vault 結構，統整散落的企劃書與對照表，形成單一真實來源的「決策日誌」。
